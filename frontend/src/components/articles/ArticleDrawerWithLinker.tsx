// src/components/articles/ArticleDrawerWithLinker.tsx
/**
 * ArticleDrawer — оновлена версія з підтримкою:
 *   1. Виділення тексту → прив'язка фрагмента до довідника
 *   2. NewsLinkerV2 замість NewsLinker
 *   3. Підсвітка виділеного тексту
 */
...
export const ArticleDrawerWithLinker = ({ articleId, onClose }: Props) => {
  const { data: article, isLoading } = useArticle(articleId);
  const bodyRef = useRef<HTMLDivElement>(null);

  // Захоплення виділеного тексту
        ...
        <>
          ...
          >
            ...
            <div className="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-b border-slate-200 dark:border-slate-200 dark:border-slate-800 flex-shrink-0">
              ...
              <div className="flex items-center gap-2">
                {/* Handbook linker — в header для зручності */}
                {article && (
                  <NewsLinkerV2
                ...
                <button
                  onClick={onClose}
                  className="hidden md:flex p-1.5 rounded-lg text-slate-400 dark:text-slate-500 dark:text-slate-400 hover:text-slate-400 dark:text-slate-600 dark:hover:text-slate-900 dark:text-white hover:bg-slate-100 dark:hover:bg-slate-100 dark:bg-slate-800 transition-colors"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
                ...
                >
                  <div className="px-4 py-2 bg-violet-500/10 border-b border-violet-500/20 flex items-center gap-2">
                    <span className="text-xs text-violet-400 flex-1 truncate">
                      ✂️ Виділено: «{selectedText.slice(0, 60)}
                      {selectedText.length > 60 ? "…" : ""}»
                    </span>
                    <button
                      onClick={clearSelection}
                      className="text-violet-500 hover:text-violet-300 text-[10px] font-mono transition-colors"
                    >
                      скинути
                    </button>
                  </div>
            ...
            <div className="flex-1 overflow-y-auto overscroll-contain">
              ...
              {article && (
                <div className="p-4 sm:p-6 space-y-4">
                  ...
                  {/* Body — з ref для захоплення виділення */}
                  <div
                    ref={bodyRef}
                  ...
                  {/* Hint */}
                  <p className="text-[11px] text-slate-400 dark:text-slate-500/60 italic">
                    💡 Виділіть текст у статті, щоб прив'язати фрагмент до
                    персони або структури довідника
                  </p>
                </div>
              )}
            </div>
