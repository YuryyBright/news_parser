...
import { hu, ro, sk, uk, pl } from "date-fns/locale";
...
export const scoreColor = (score: number): string => {
  ...
  return "text-slate-400 dark:text-slate-500 bg-slate-50 dark:bg-slate-100 dark:bg-slate-800 dark:text-slate-400 dark:text-slate-500 border-slate-200 dark:border-slate-300 dark:border-slate-700";
};
...
export const statusColor = (status: string): string => {
  const map: Record<string, string> = {
    ...
    expired:
      "text-slate-400 dark:text-slate-500 bg-slate-50 dark:bg-slate-100 dark:bg-slate-800 dark:text-slate-400 dark:text-slate-500 border-slate-200",
    processing:
    ...
    read: "text-slate-400 dark:text-slate-500 bg-slate-50 dark:bg-slate-100 dark:bg-slate-800 border-slate-200",
  };
  return (
    map[status] ??
