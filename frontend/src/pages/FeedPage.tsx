              ...
              <div>
                ...
                <input
                  ...
                  className={cn(
                    ...
                    "[&::-webkit-slider-thumb]:bg-slate-50 dark:bg-slate-900 dark:[&::-webkit-slider-thumb]:bg-white",
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            ...
                            active
                              ? "bg-slate-50 dark:bg-slate-900 dark:bg-white border-slate-900 dark:border-white"
                              : "border-slate-200 dark:border-slate-300 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-400 dark:hover:border-slate-600",
                          )}
                        >
...
export const FeedPage = () => {
  ...
  const unreadItems = allItems.filter((item: any) => !checkIsRead(item));
  const unreadCount = feedFilter === "read" ? 0 : unreadItems.length;

  const availableLangs = Array.from(
    new Set(
      allItems.map((i: any) => i.language?.toLowerCase()).filter(Boolean),
    ),
  ) as string[];

  const availableTags = Array.from(
    new Set(allItems.flatMap((i: any) => i.tags ?? []).filter(Boolean)),
  ) as string[];

  // Per-lang unread counts for tab bar
  const unreadLangCounts = availableLangs.reduce(
    (acc, lang) => {
      acc[lang] = unreadItems.filter(
        (i: any) => i.language?.toLowerCase() === lang,
      ).length;
      return acc;
    },
  ...
  const langFilteredPool =
    feedFilter === "unread" && activeLang && filters.langs.length === 0
      ? basePool.filter((i: any) => i.language?.toLowerCase() === activeLang)
      : basePool;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {unreadCount > 0 && (
            <button
              onClick={() =>
                markAllRead.mutate(unreadItems.map((i: any) => i.article_id))
              }
          ...
          >
            ...
            {activeFilterCount > 0 && (
              <span className="inline-flex items-center justify-center w-4 h-4 rounded-full text-[10px] font-bold bg-blue-500 text-slate-900 dark:text-white">
                {activeFilterCount}
              </span>
            )}
          </button>
        ...
        {FEED_FILTER_TABS.map(({ key, label }) => (
          ...
          >
            ...
            {key === "unread" && unreadCount > 0 && (
              <span className="ml-1.5 inline-flex items-center justify-center w-4 h-4 rounded-full text-[10px] font-bold bg-blue-500 text-slate-900 dark:text-white">
