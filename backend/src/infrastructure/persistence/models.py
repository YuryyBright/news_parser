# infrastructure/persistence/models.py  (SQLite версія — без Vector колонок)
...
class SourceModel(Base):
    ...
    fetch_interval_sec: Mapped[int]      = mapped_column(Integer, default=lambda: settings.celery.fetch_interval_seconds)
    is_active:          Mapped[bool]     = mapped_column(Boolean, default=True)
    created_at:         Mapped[datetime] = mapped_column(TIMESTAMP, default=utcnow)
...
class FilterCriteriaModel(Base):
    ...
    phrases:           Mapped[str]   = mapped_column(JSON, default=list)   # JSON array
    keywords:          Mapped[str]   = mapped_column(JSON, default=list)
    allowed_languages: Mapped[str]   = mapped_column(JSON, default=list)
    threshold:         Mapped[float] = mapped_column(Float, default=lambda: settings.filtering.default_threshold)
    ...
    weights:           Mapped[dict]  = mapped_column(JSON, default=lambda: {
        "embedding": settings.filtering.weights.embedding,
        "keyword":   settings.filtering.weights.keyword,
        "feedback":  settings.filtering.weights.feedback,
    })
    updated_at: Mapped[datetime] = mapped_column(TIMESTAMP, default=utcnow, onupdate=utcnow)
...
class FeedSnapshotModel(Base):
    ...
    items: Mapped[list["FeedItemModel"]] = relationship(back_populates="snapshot")
...
class FeedItemModel(Base):
    ...
    article:  Mapped["ArticleModel"]      = relationship(back_populates="feed_items")
